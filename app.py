...
def session_required(f):
    ...
    def decorated(*args, **kwargs):
        if session.get("authenticated"):
            # যদি এক্সপাইরি হয়ে যায়, সেশন ক্লিয়ার করে রিডাইরেক্ট
            if is_expired():
                session.clear()
...
# ============================================
# USER DASHBOARD (সংক্ষিপ্ত, কিন্তু নম্বর ইনফরমেশন সহ)
# ============================================
USER_PANEL_HTML = '''
<!DOCTYPE html>
<html lang="en">
...
# ============================================
# ADMIN DASHBOARD HTML (বর্ধিত—এক্সপাইরি ম্যানেজমেন্ট সহ)
# ============================================
...
<body>
    ...
    <script>
        ...
        // Set initial datetime-local value from current expiry (if available)
        document.addEventListener('DOMContentLoaded', function() {
            // Pre-fill with current expiry if available (we'll get from server)
            // already set in template via expiry_local
        });
    </script>
</body>
</html>
...
def login_page():
    if request.method == 'POST':
        ...
        if verify_user(username, password):
            ...
            # Wrong password or inactive user -> redirect
            return redirect('https://wa.me/919242428894')
...
def admin_dashboard():
    ...
    # For datetime-local input, we need local time (Render uses UTC, but we show UTC to avoid confusion)
    settings = get_settings()
...
def admin_set_expiry():
    ...
    try:
        ...
        # Convert to UTC string with +00:00
